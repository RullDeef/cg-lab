...
namespace core
{
    ...
    {
    public:
        explicit BaseCutter(const char* name) : NamingStrategy(name) {}
    ...
    {
    public:
        explicit ConvexCutter(const char* name) : NamingStrategy(name) {}
        virtual ~ConvexCutter() = default;
        virtual void cut(std::list<Segment>& lines, const BasicRegion& cutter) = 0;
    };

    class PolyCutter : public NamingStrategy, public TimingStrategy
    {
    public:
        explicit PolyCutter(const char* name) : NamingStrategy(name) {}
        virtual ~PolyCutter() = default;
        virtual void cut(BasicRegion& region, const BasicRegion& cutter) = 0;
    };
}
